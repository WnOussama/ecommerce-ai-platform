...
| 12 | The app connects to Postgres as `saas_user`, a **superuser** and the table owner. | Tested live | Create a limited application role (no superuser, no owner, only the grants it needs). Run migrations with a separate owner role. This is also the first step for row level security (section 3). |
